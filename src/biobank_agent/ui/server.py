...
class StudyConsoleStore:
    ...
    def start_new_session(self, requested_by: str) -> dict[str, Any]:
        ...
        if state.get("status") not in {"COMPLETED", "FAILED"}:
            raise RuntimeError(
                "A new session can be started only after completion or failure "
...
function render(s){const state=s.state||{};const status=state.status||'STARTING';currentStatus=status;$('status').textContent=status;show('question-card',status==='WAITING_FOR_QUESTION');show('waiting-card',['STARTING','DISCOVERING','PLANNING','APPROVED','DISPATCHING_ANALYSIS'].includes(status));$('waiting-text').textContent={STARTING:'Waiting for the NVFlare Controller.',DISCOVERING:'Site agents are generating and verifying adapters from disclosure-controlled local profiles; no patient rows leave a site.',PLANNING:'Codex is translating the verified client adapters into a typed contract using the registered tools.',APPROVED:'Approval validated. Preparing dispatch.',DISPATCHING_ANALYSIS:'Approved analysis is executing behind each site boundary.'}[status]||'';
...
$('tools').replaceChildren(...(p.analyses||[]).map(a=>{const d=document.createElement('div');d.className='item';const dynamic=(p.dynamic_tools||[]).find(tool=>tool.name===a.tool);const b=document.createElement('strong');b.textContent=`${a.analysis_id||'analysis'} · ${a.tool}${dynamic?' · AGENT-GENERATED':''}`;const q=document.createElement('div');q.textContent=dynamic?.description||registry[a.tool]?.description||'';const pre=document.createElement('div');pre.className='muted';pre.textContent=Object.entries(a).filter(([k])=>!['analysis_id','tool'].includes(k)).map(([k,v])=>`${k}: ${JSON.stringify(v)}`).join(' · ');d.append(b,q,pre);if(dynamic){const details=document.createElement('details');const summary=document.createElement('summary');summary.textContent='Review approval-bound generated source';const code=document.createElement('pre');code.className='code-review';code.textContent=`LOCAL\n${dynamic.local_code}\n\nSERVER\n${dynamic.server_code}`;details.append(summary,code);d.append(details)}return d}));
...
$('sites').replaceChildren(...(f.sites||[]).map(site=>{const tr=document.createElement('tr');const adapter=site.data_adapter||{};const adapterText=adapter.status?`${adapter.status} · ${(adapter.digest||'').slice(0,10)}`:'unavailable';const supportStatus=!executable?'No executable analysis':site.support_status==='not_targeted'?'Not targeted':site.all_requested_analyses_supported?'Supported':'Partial';const fedready=executable&&(site.fedready===true||(site.fedready===undefined&&f.sites.every(item=>item.all_requested_analyses_supported)))?1:0;[site.site_id,adapterText,site.supported?.length||0,fedready,site.unsupported?.map(x=>x.analysis_id).join(', ')||'0',supportStatus].forEach((v,i)=>{const td=document.createElement('td');td.textContent=v;if(i===3)td.className=fedready?'ok':'no';if(i===5)td.className=site.all_requested_analyses_supported&&executable?'ok':'no';tr.append(td)});return tr}));
...
$('start-new-completed').onclick=async()=>{if(currentStatus==='WAITING_FOR_QUESTION'){message('A blank research question is already ready.');$('question').focus();return}if(!window.confirm('Start a fresh study? The current attempt will remain archived.'))return;try{if(['COMPLETED','FAILED'].includes(currentStatus)){const result=await post('/api/start-new-session',{requested_by:'Cecilie'});message(`Started ${result.session_id}. Waiting for the new Controller…`)}else if(currentStatus==='WAITING_FOR_APPROVAL'){await post('/api/new-question',{requested_by:$('approver').value});message('Current proposal archived. Reopening the question form…')}else throw Error('Start New is available at the next safe human checkpoint.');refresh()}catch(e){message(e.message,true)}};
